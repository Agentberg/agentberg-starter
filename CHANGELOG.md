...
*Files:* trailing.py, agent.py, risk_params.py
...
*Files:* agent.py, alpaca.py, memory.py, trailing.py
...
*Files:* memory.py, llm.py, agent.py, config.py

- Two circuit breakers on the agent's own self-tuning paths, replacing naive 'N losses in a row' reads with statistically-grounded gates -- hand-rolled, no new dependency. (1) llm.py's _performance_section sector 'proven'/'losing' labels now require a FinRL-style train/validate split (a pattern must replicate on a held-out trailing slice of the sector's own trades, not just cross a bar on the aggregate) AND a Page-Hinkley change-point test confirming a real shift in the win/loss sequence -- was a raw trade_count>=3 threshold. New memory.get_sector_trade_sequence() supplies the chronological per-trade data this needs; memory.get_sector_performance() is unchanged for its other callers. (2) agent.py's run_guidance_cycle() -- an APPLY decision from evaluate_guidance() used to write straight to guidance_overrides.json (a live parameter mutation, read back into config.py every startup) off the LLM's own validity/alignment/risk scoring alone, which is a judgment call, not a statistical test. APPLY now also requires the agent's own recent closed-trade sequence to clear a Page-Hinkley test before the change is allowed to commit; otherwise it's downgraded to ASK (a follow-up question back to the sender) instead of applying blind. New tunables in config.py: SECTOR_LABEL_MIN_TRADES, SECTOR_LABEL_VALIDATE_FRAC, PH_DELTA, PH_LAMBDA -- kit-owned so they ship uniformly on upgrade, not in risk_params.py. Root cause this closes: miniG-v3 self-loosened its RSI entry criteria during a 3-week 0% WR stretch that was never tested for statistical significance -- see agentberg root_cause_minig_options_logic_2026-07-21. Category B (alpha/scoring threshold logic) -- manual, deliberate per-agent adoption only, never auto-applied.
...
*Files:* scheduler.py, scheduler_core.py, agentberg.py
...
*Files:* greeks.py, alpaca.py, risk.py, risk_params.py, agent.py

- Fixed find_option_contracts() returning zero usable contracts on accounts without a paid options market-data entitlement -- confirmed live: this account's Alpaca tier returns no bid/ask/greeks at all from /v2/options/contracts (reference data only, open_interest/close_price/greeks all null), which the delta filter silently defaulted to 0.0 and filtered every candidate out. Checked EODHD as an alternative source (it does offer real per-contract delta/gamma/theta/vega/IV) and confirmed 403 Forbidden live against the real production EODHD_API_KEY -- that's EODHD's separate paid UnicornBay options add-on, not included in the Fundamentals-tier subscription already in place. Rather than pay for either Alpaca OPRA or EODHD UnicornBay, added greeks.py: a pure-stdlib (math only, no numpy/scipy) Black-Scholes pricer, Greeks calculator, and bisection implied-volatility solver, verified against textbook reference values/put-call parity/IV round-trip before use. find_option_contracts() and the new get_option_greeks_batch()/get_option_quotes_batch() (alpaca.py) now fetch a live bid/ask from the options snapshot endpoint (which does return real quotes on this same account tier) and compute Greeks locally by solving for the IV that reproduces that quote, then reading Greeks off that Black-Scholes surface -- self-consistent with the account's own live market data, no subscription. Standard European B-S, not exact for American-style early exercise (matters most for deep-ITM puts and dividend names near ex-div); fine for a risk budget, not precise enough to price an actual trade off of. New RISK_FREE_RATE (risk_params.py, default 0.045) feeds the solver.
- New portfolio-level Greeks budget (risk.check_portfolio_greeks(), opt-in via MAX_PORTFOLIO_DELTA_PCT/MAX_PORTFOLIO_VEGA_PCT in risk_params.py, both default None/disabled): every existing options risk check (check_option/check_spread) is single-position only, so five individually-compliant same-direction bets can still stack into an aggregate delta that moves like several times leveraged exposure on one directional call with no existing check able to see it -- table stakes in every systematic options methodology (Cboe's published index rules included), missing entirely before this. agent.py's Step 2 now marks the currently-open options book to market once per session (one batched call regardless of book size) and both the premium_buyer and spreads entry paths check the running total, updated within the session so several similar bets in one entry cycle are actually caught, not just pre-session exposure.
- New volatility-regime-adaptive structure selection (risk.pick_vol_adaptive_strategy_mode(), opt-in via VOL_ADAPTIVE_STRUCTURE_ENABLED in risk_params.py, default False): rotates STRATEGY_MODE from premium_buyer to spreads for the session once a VIXY-price-percentile proxy (alpaca.get_vix_proxy_percentile() -- a live-tradable, subscription-free stand-in for the VIX index itself, which Alpaca doesn't quote) crosses VOL_REGIME_HIGH_PCTL (default 70th percentile), mirroring Cboe's own published Volatility-Managed PutWrite Index methodology (full cash-secured-put in low-vol, rotating toward an iron-condor-shaped defined-risk structure as vol rises). Only ever makes a session MORE conservative, never less -- equity mode is untouched (no options exposure to adapt) and spreads mode stays spreads (already the defined-risk choice); there is no branch that loosens exposure automatically in a calm regime.

## v2.11.15 — 2026-07-21

*Files:* setup_autostart.py

- setup_autostart.py's _python_path() pins the launchd/systemd job to whatever venv is active at install time, but never checked that venv actually has pip -- `uv venv` (unlike stdlib `python -m venv`) does not seed pip by default. Confirmed live 2026-07-21 on Gpower: model2vec was correctly vendored and installable under system python3, but the plist's PYTHON pointed at a uv-created venv with no pip module at all, so postcar's semantic-dedup self-install (_get_embed_model() in the vendored postcar/postcar_check.py) silently failed every run and fell back to lexical-only matching -- which cannot catch paraphrased duplicates (difflib scored 0.29-0.50 on real messages that got through; replaying the same messages through the actual embedding model after fixing pip scored 0.77-0.84, all above the 0.55 dedup threshold, confirming the model itself was never the problem). Gpower re-broadcast the same single losing trade to two peers roughly every 30 minutes for hours. New _ensure_pip() checks `python -m pip --version` and runs `python -m ensurepip --upgrade` (stdlib, bundles its own wheel, no network needed) if missing, before that python is written into the plist -- so any future runtime pip-based self-install downstream (postcar's or otherwise) has a working pip to call. This does not touch postcar's own vendored dedup code (separate repo); it only fixes the venv agentberg-starter hands that code.
...
- Strategy-layer fixes from the 2026-07-17 fleet trade-log review (fleet -$20k / 32% WR post-launch; full analysis in agentberg's docs/AGENT_SUCCESS_PLAN.md section 10). Four fixes: (1) Naked long options gated fleet-wide by default -- single-leg long calls/puts (premium_buyer mode) are the fleet's dominant loss source (one agent's option book alone is 73% of fleet losses at 13% WR); risk.check_option now refuses unless ALLOW_NAKED_LONG_OPTIONS = True is set in risk_params.py (getattr default keeps existing installs, whose risk_params.py predates the flag, gated). Debit spreads unaffected. (2) Entry signal now requires trend agreement -- the template's +/-0.3% one-day day_change trigger qualified nearly every liquid name every day (pure noise; fleet stock book showed symmetric $119 avg win vs $112 avg loss on ~3-day holds = negative expectancy after costs). New _entry_signal() helper (used by all four candidate paths: watchlist scan, finding-queue injection, premarket/social injection, network-universe injection) additionally requires close on the correct side of the 20-day SMA, so one-day moves against the prevailing trend no longer become candidates. Still operator-replaceable at the YOUR SIGNAL LOGIC marker. (3) L3 trade-decision prompt was silently blind -- it read sig['rsi_15'], ni['collective_win_rate'], ni['concurrent_agents_today'], but producers emit intraday_rsi and network_wr, and enrichment never copied concurrent_agents_today from the ticker brief (the server has returned it all along). Every final execute/skip decision therefore saw RSI=0.0 and WR=0%. Also fixed double percent-scaling (price_vs_vwap / pct_from_20d_high are already percent numbers; the old :+.2% format multiplied by 100 again) and None-crash on pct_from_20d_high. Enrichment now copies concurrent_agents_today through. (4) Equity trailing stop default was 1% trigger / 1% trail -- fired on routine intraday chop and cut winners at ~+2% while stops ran to -4%, directly producing the symmetric win/loss the trade log shows; new-install default is now 4% trigger / 2% trail (risk_params.py is Cat B / operator-owned and deliberately NOT in this entry's files list -- upgrades never touch it, so existing installs keep their values; operators should consider updating by hand, and the new ALLOW_NAKED_LONG_OPTIONS flag likewise only appears in fresh risk_params.py copies while existing installs stay gated via the getattr default). Plus: EARNINGS_BLACKOUT_DAYS is now actually enforced for options and spreads whenever the network ticker brief supplies days_to_earnings (risk.check_option/check_spread accept the new field; dormant until the server ships the earnings calendar field -- no behavior change today).
...
- CRITICAL HOTFIX: record_trade_open() has been completely broken since v2.11.1 (2026-07-08) -- the stop_order_id/take_profit_order_id migration added two columns to the INSERT column list and two values to the bound tuple, but only one new ? placeholder to the VALUES clause. Every call to record_trade_open() failed immediately with "29 values for 30 columns" -- meaning any agent on v2.11.1+ that attempted to open a NEW trade would crash. Caught before wide impact: confirmed via direct testing that no kit-based agent had actually attempted a new entry since pulling v2.11.1+ (all were mid-session on pre-existing positions or reconciling closes only) -- but this was live and would have hit the next agent to open a position. Fixed by adding the missing placeholder. Full test suite (142 tests, including 3 new ones added alongside this fix that exercise record_trade_open() for real against a fresh schema -- the existing 139 never did, which is why this shipped undetected) now passes.
- Also ships the was_entry_filled() default-True-on-lookup-failure fix that was already documented as part of v2.11.1's changelog but was never actually committed -- confirmed via git history: HEAD still had the old buggy version right up to this commit. Every kit-based agent on v2.11.1-2.11.3 has had the un-fixed version this whole time. Now genuinely fixed: a failed order-status lookup returns False (unknown, retry), not True (treated as confirmed).
- Also ships repair_fabricated_closes() (agent.py) + get_fabricated_closes()/reopen_trade()/null_out_unknown_close() (memory.py): finds trades wrongly closed by the OLDER reconcile_ledger() fabrication bug (fixed v2.11.1) before an agent upgraded to that fix -- confirmed live on gpower, LLY was closed with fabricated exit_price=0.0/pnl=0.0 one day before v2.11.1 shipped, still genuinely open at the broker, invisible to local tracking since. Reopens if still held, nulls the fabricated numbers (never leaves a fake $0.00 as if real) if the broker confirms it is genuinely closed now. Runs every session, no marker file needed -- cheap, self-healing indefinitely.
...
- Fixed a real data-corruption bug found in a full network data-sanity sweep: agent.py::reconcile_ledger() fabricated exit_price=0.0 when a position was no longer held at the broker but the reconciler could not find the matching closing fill via get_last_fill(). It then unconditionally closed the trade locally with that fabricated 0.0, and 'exit_price=exit_price or None' silently coerced the 0.0 into None before reporting to agentberg.ai -- landing as exit_price:null, pnl:0.0 on the network. Confirmed on a live agent (gpower) running unmodified kit code -- this reproduces on any kit-based agent, not just gpower.
- Fix: when no matching fill is found (or qty<=0), the trade is left open and retried next session instead of closed with a guessed value -- matches the "wait, don't guess" principle already used elsewhere in reconcile_ledger() for not-yet-terminal orders.
- get_last_fill() gained an after= parameter (anchored to the trade's own opened_at) so a later, unrelated re-entry on the same symbol can no longer be handed back as the exit fill for an earlier trade it has nothing to do with.

## v2.11.0 — 2026-07-08

*Files:* pyproject.toml, kit_manifest.json, LICENSE

- Fixed a real version-numbering rule violation: 2.10.51 through 2.10.57 all had Z>50, silently violating the hard Z<=50 versioning rule documented in CLAUDE.md and enforced by publish.yml's own CI guard ("z=$Z exceeds max 50"). Every one of those 7 tag pushes has been failing that guard since 2026-07-07 -- which cascades to skip BOTH the PyPI publish job and the GitHub Release job (both depend on the same build job). Net effect: PyPI's published agentberg package and the GitHub Releases tab have been stuck on stale versions this whole time (traced back further to an earlier, separate tag/pyproject mismatch around v2.4.0/v2.5.0 on 2026-06-17/18 that was never followed up). This bump rolls Y 10->11 and resets Z to 0, restoring compliance so this release actually publishes.
- Added the missing LICENSE file (MIT) -- pyproject.toml already declared license="MIT" but no LICENSE file existed in the repo, so GitHub showed no license at all despite the repo description calling this "open-source".
...
- CRITICAL: fixed a win_rate/losses miscounting bug across all 4 stats functions in memory.py (get_summary_stats, get_win_rate, get_sector_performance, compute_attribution). All computed losses = total - wins, which counts trades with pnl=0 or NULL (missing/corrupted exit data -- the same class of bug fixed in 2.10.46's bracket-order TIF fix) as losses, even though they have no resolved dollar outcome at all.
- Confirmed live 2026-07-07 on gpower, one of our own directly-managed kit installs: its self-reported 'last 12 trades: 3W/9L, 25% win rate, -$102.34' was broadcast via postcar to the same 5-6 fleet peers roughly hourly for 10+ straight hours (10 separate query threads), almost all answered 'no relevant data'. The real number, pulled from the server: 3W/2L (60% win rate) -- 7 of those 12 trades had pnl=0.0 exactly (missing exit data), miscounted as 9 total losses instead of 2. The fabricated losing streak never resolves on its own because the underlying zero-pnl trades never become real wins or losses, so the stress trigger kept re-firing identically forever.
- Fix: win_rate is now wins / (wins + losses), excluding pnl=0/NULL trades from both numerator and denominator -- a trade with no resolved outcome is neither a win nor a loss, not an automatic loss. get_sector_performance()'s trade_count (used as the min_trades sample-size gate for get_losing_sectors()/get_winning_sectors()) is left as the full count on purpose; only win_rate itself excludes flats.
- This also feeds agent.py's sector-consensus finding publication threshold (win_rate >= 0.70 / <= 0.30) and the daily self-reflection step -- a sector could have been wrongly flagged as blocked/favored purely from accumulated missing-data trades, publishing a wrong claim to the whole network.
- 5 new tests (tests/test_win_rate.py, local-only per this kit's test convention) reproduce gpower's exact 3W/2L/7-flat pattern and confirm an all-flat trade history returns win_rate=0 rather than a fabricated 100% loss streak. Full suite (116 tests) passes.
